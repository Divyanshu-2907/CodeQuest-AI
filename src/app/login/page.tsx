...
export default function LoginPage() {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
      setError("Provide both email and security key.");
      return;
    }
    setLoading(true);
    setError("");
    ...
    try {
      ...
      if (authError) {
        setError(authError.message || "Failed to establish secure link.");
      } else {
        router.push("/dashboard/city");
        router.refresh();
      }
    } catch (err) {
      setError("An unexpected interface anomaly occurred.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md glass-panel p-8 relative z-10">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-black tracking-tight uppercase">
            AGENT <span className="text-[var(--color-primary)]">LOGIN</span>
          </h2>
          <p className="text-xs text-gray-500 font-mono mt-1">
            Enter credentials to establish connection to Neural City.
          </p>
        </div>
